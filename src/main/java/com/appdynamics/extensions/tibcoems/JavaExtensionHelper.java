...
public abstract class JavaExtensionHelper extends AManagedMonitor {
	...
	public abstract TaskOutput execute(Map<String, String> taskArguments,
	...
	protected void printMetric(String name, String value, String aggType,
			String timeRollup, String clusterRollup) {
		String metricName = getMetricPrefix() + name;
		MetricWriter metricWriter = getMetricWriter(metricName, aggType,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("METRIC:  NAME:" + metricName + " VALUE:" + value
					+ " :" + aggType + ":" + timeRollup + ":" + clusterRollup);
		}
	}
	...
	// lookup value for key, convert to float, round up or down and then return
	// as string form of int
	protected String getString(String key) {
		return getString(key, true);
	}

	// specify whether to convert this key to uppercase before looking up the
	// value
